...
type TaskChipProps = TaskChipBaseProps & TaskChipCategoryProps;
...
export function TaskChip(props: TaskChipProps) {
    const {
        ...
        onClick,
        backgroundClassName = "bg-black-850",
    } = props;
    ...
    const categoryPresentation =
        props.category !== undefined
            ? getCategoryPresentation(props.category)
            : {
                  color: props.categoryColor,
                  icon: CATEGORY_ICON_MAP[props.categoryIconKey],
              };

    const clampedProgressRate = Math.min(100, Math.max(0, progressRate));

    const progressLevel = Math.floor(clampedProgressRate / 10);

    const progressOffset =
        PROGRESS_CIRCUMFERENCE * (1 - clampedProgressRate / 100);
    ...
    rounded-[12px] ${backgroundClassName}
