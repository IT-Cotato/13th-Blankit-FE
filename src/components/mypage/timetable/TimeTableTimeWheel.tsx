    ...
    <div
      ref={(node) => {
        ...
        if (node && !initializedRef.current) {
          const initialIndex = circular ? middleSetStart + value : value;
      ...
      onScroll={(event) => {
        ...
        if (
          circular &&
          (displayIndex < options.length || displayIndex >= options.length * 4)
        ) {
          const centeredIndex = middleSetStart + optionIndex;
          requestAnimationFrame(() => {
            if (wheelRef.current) {
              wheelRef.current.scrollTop = centeredIndex * ITEM_HEIGHT;
      ...
      {displayedOptions.map(({ label }, index) => (
        <button
          ...
          className={`flex h-8 w-full snap-center items-center justify-center text-lg font-medium tracking-[-0.27px] ${
            index === activeDisplayIndex ? "text-black-100" : "text-black-700"
          }`}
          style={{
            opacity: Math.max(0.18, 1 - Math.abs(index - activeDisplayIndex) * 0.28),
...
export function TimeTableTimeWheel({
  ...
  const initialStartMinutes = startHour * 60 + (initialEntry?.startSlot ?? 0) * 5;
  const initialEndMinutes =
  ...
  const [startHourIndex, setStartHourIndex] = useState(
    Math.max(0, Math.min(hours.length - 1, Math.floor(initialStartMinutes / 60) - startHour)),
  );
  ...
  const [endHourIndex, setEndHourIndex] = useState(
    Math.max(0, Math.min(hours.length - 1, Math.floor(initialEndMinutes / 60) - startHour)),
  );
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/60">
      <section
        className="flex w-full flex-col items-start gap-5 rounded-t-xl bg-black-850 px-5 pb-5 pt-[21px]"
