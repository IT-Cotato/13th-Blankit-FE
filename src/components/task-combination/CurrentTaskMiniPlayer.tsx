...
import { formatTimer, getTaskProgress } from "@/utils/taskTimer";
...
export function CurrentTaskMiniPlayer({
    ...
    const hasStarted = usePlaylistStore((state) => state.hasStarted);
    ...
    const {
        ...
        isPlaying,
        toggleTimer,
    } = useCurrentTaskTimer(task.estimatedMinutes);
    const { recommendedMinutes, recommendationTimeError } =
        useTodayRecommendedMinutes(dailyRecommendationRefreshKey);
    const {
        session,
        isLoadingSession,
        isUpdatingSession,
    ...
    const missingTaskIdMessage = getMissingTaskIdMessage(task.taskId);
    ...
    useEffect(() => {
        if (!session || !shouldRestoreTimerFromSession(hasStarted)) {
            return;
        }
    ...
    const handleToggleTimer = async () => {
        if (!session || isLoadingSession || isUpdatingSession) {
            return;
        }
